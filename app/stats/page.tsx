...
export default async function StatsPage() {
  const [matchLeaders, winLeaders, titleLeaders, rivalries, countryWins, threeSetStats, siteStats] = await Promise.all([
    getCareerMatchLeaders(),
    getCareerWinLeaders(),
    getTitleLeaders(),
    getGreatestRivalries(),
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-14">
        ...
        <StatCard label="3-Set Rate" value={`${globalThreeSetPct}%`} sub="of all matches go 3 sets" />
        <StatCard
          label="Top Nation"
      ...
      <section className="mb-14">
        <SectionHeader
          ...
          sub="Most-played head-to-head pairings of all time"
        />
      ...
      {/* 3-Set Breakdown by Category */}
      <section className="mb-14">
        <SectionHeader
          title="3-Set Battle Rate by Discipline"
          sub="How often does each discipline go to a deciding set?"
        />
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
          {threeSetStats.map((row) => {
            const pct = Number(row.total) > 0
              ? Math.round((Number(row.three_set) / Number(row.total)) * 100)
              : 0
            return (
              ...
              <div key={gi} className="divide-y divide-white/5">
                {group.map((row, i) => {
                  ...
                  return (
                    <Link
                      key={row.country}
                      href={`/players?country=${row.country}`}
